...
class SteamClient {
  ...
  private setRequestInterception = async (enable: boolean) => {
    ...
    if (this.page != null) {
      ...
      (await this.page).on("request", (request: { resourceType: () => string; abort: () => void; continue: () => void }) => {
  ...
  public getNewComments = async (profileUrl: string): Promise<Comment[] | null> => {
    ...
    if (this.page !== undefined) {
      ...
      comments.some(async (comment) => {
        ...
        if (commentTimestamp && (Math.round(new Date().getTime() / 1000) - interval).toString() < commentTimestamp) {
          ...
          if (commentText?.includes("This comment is awaiting analysis")) {
            ...
            if (commentAuthor && username && commentText) {
